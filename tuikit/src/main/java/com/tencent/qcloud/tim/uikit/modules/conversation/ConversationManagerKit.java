...
public class ConversationManagerKit implements TIMRefreshListener, MessageRevokedManager.MessageRevokeHandler {
    ...
    public void clearUnRead(){
        ...
        for (int i = 0; i < TIMConversations.size(); i++) {
            final TIMConversation conversation = TIMConversations.get(i);
            conversation.setReadMessage(null, new TIMCallBack() {
                @Override
                public void onError(int i, String s) {
                    Log.e("xld",conversation.getPeer()+":消息已读上报失败,error:"+s);
                }

                @Override
                public void onSuccess() {
                    Log.e("xld",conversation.getPeer()+":上报消息已读");
                }
            });
        }
        onRefreshConversation(TIMConversations);
    }
    ...
    public void loadConversation(IUIKitCallBack callBack) {
        ...
        for (int i = 0; i < TIMConversations.size(); i++) {
            ...
            if (conversationInfo != null) {
                if (conversationInfo.getType() == TIMConversationType.C2C.value()) { //未读消息计数只记录私聊会话
    ...
    private List<ConversationInfo> sortConversations(List<ConversationInfo> sources) {
        ...
        for (int i = 0; i <= sources.size() - 1; i++) {
            ...
            if (isTop(conversation.getId())) {
                ...
                if (!conversation.isGroup()) { //不加载群聊消息
